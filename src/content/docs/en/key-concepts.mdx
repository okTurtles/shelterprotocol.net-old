...
We can use these contract chains to create and define concepts in our programs in a similar way to how Classes and Objects are used in [Object-oriented Programming](https://en.wikipedia.org/wiki/Object-oriented_programming), except these Classes and Objects can shared across many devices while keeping their data end-to-end encrypted.
...
The first event defines and initializes the contract (or "class instance" in OOP terms). Each subsequent event invokes actions that update the state for that contract instance. Because developers make mistakes, contracts are upgradeable, and every event [references the specific version](#contract-manifests) of the contract that should be used to interpret that event. This guarantees that the final state that results from processing all of the events will be the same no matter when the events are processed.
...
📚 [**Reference: Contract Manifest**](contract-manifest)
...
- [`OP_KEY_REQUEST_RESPONSE`](opcodes#op_key_request_response)
- [`OP_KEYSHARE`](opcodes#op_keyshare)
- [`OP_CONTRACT_AUTH`](opcodes#op_contract_auth)
- [`OP_CONTRACT_DEAUTH`](opcodes#op_contract_deauth)
